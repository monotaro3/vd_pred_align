...
class MultiboxTrainChain(chainer.Chain):
    ...
    def __init__(self, model, alpha=1, k=3,mbox_another=None):
        super(MultiboxTrainChain, self).__init__()
        with self.init_scope():
            self.model = model
            if mbox_another:
                self.mbox_another = mbox_another
            else:
                self.mbox_another = None
        self.alpha = alpha
        self.k = k
    ...
    def __call__(self, imgs, gt_mb_locs, gt_mb_labels):
        ...
        if self.mbox_another:
            mb_locs_, mb_confs_ = self.mbox_another(self.model.extractor(imgs))
            loc_loss_, conf_loss_ = multibox_loss(
                mb_locs_, mb_confs_, gt_mb_locs, gt_mb_labels, self.k)
            loss_ = loc_loss_ * self.alpha + conf_loss_

            chainer.reporter.report(
                {'loss': loss, 'loss/loc': loc_loss, 'loss/conf': conf_loss,
                 'loss_': loss_, 'loss/loc_': loc_loss_, 'loss/conf_': conf_loss_},
                self)
...
class Transform(object):
    ...
    def __call__(self, in_data):
        ...
        if self.transform and self.original:
            if self.use_fp16:
                img_original = img_original.astype(np.float16)
            return img, mb_loc, mb_label, img_original
        else:
...
def main():
    ...
    tchain_args = {"model" : model}
    if args.MCD:
        mbox_another = model.multibox.__class__(len(vehicle_classes)+1, model.multibox.aspect_ratios)
        tchain_args["mbox_another"] = mbox_another
    train_chain = MultiboxTrainChain(**tchain_args)
    if gpu >= 0:
    ...
    printreport_args = {"entries":['epoch', 'iteration', 'lr',
         ...
         'validation/main/map']}
    if args.MCD:
        printreport_args["entries"] += ['main/loss_', 'main/loss/loc_', 'main/loss/conf_']
    ...
    if args.MCD:
        trainer.extend(
            extensions.snapshot_object(mbox_another, 'mbox_another_iter_{.updater.iteration}'),
            trigger=(args.iteration, 'iteration'))
    ...
    if args.save_model:
        serializers.save_npz("ssd_model_{}".format(args.save_model), trainer.updater._optimizers["main"].target.model)
